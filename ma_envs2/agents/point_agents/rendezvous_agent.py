...
class PointAgent(Agent):
    ...
    #def get_observation(self,col_states, dm, my_orientation, their_orientation, vels, nh_size):
    def get_observation(self,  dm, my_orientation, their_orientation, vels, nh_size):
        if self.obs_mode == 'fix_acc':
            ...
            local_col = self.get_collison_obs()
            fix_obs = np.zeros(self.dim_rec_o)
            ...
            obs = np.hstack([fix_obs.flatten(), local_obs.flatten(),local_col.flatten()])
    ...
    def get_local_obs(self):
        ...
        return local_obs
    def get_collison_obs(self,colstates):
        local_col=np.zeros(3)
        col_dists=np.zeros(self.nr_col)
        col_angles = np.zeros(self.nr_col)
        for i in range(self.nr_col):
            delta=self.state.p_pos - colstates[i]
            col_dists[i]=np.sqrt((np.abs(delta) ** 2).sum(axis=-1))
            col_angles[i]=np.arctan2(delta[:, 1], delta[:, 0])
            closest_col = np.argmin(col_dists)
            local_col[0] = col_dists[closest_col]
            local_col[1] = np.cos(col_angles[closest_col])
            local_col[2] = np.sin(col_angles[closest_col])
        return local_col