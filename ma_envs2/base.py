...
dynamics = ['point', 'unicycle', 'box2d', 'direct', 'unicycle_acc','unicycle_wheel']
...
class Collides(Entity):
    def __init__(self):
        super(Landmark,self).__init__()
    def reset(self,state,radius):

        self.state.p_pos=state[0:2]
        self.state.size=radius
...
class Agent(Entity):
    ...
    def __init__(self):
        ...
        self.max_lin_velocity = 14.3  # cm/s
...
class World(object):
    ...
    def step(self):
        ...
        elif self.agent_dynamic == 'unicycle_wheel':
            # unicycle dynamics

            scaled_actions = np.zeros([self.nr_agents, 2])
            for i, agent in enumerate(self.policy_agents):
                # 不需要*max_lin_velocity
                scaled_actions[i, 0] = (agent.action.u[0] + agent.action.u[1])*0.075344* 100
                scaled_actions[i, 1] = (agent.action.u[1] - agent.action.u[0])*2.65062
